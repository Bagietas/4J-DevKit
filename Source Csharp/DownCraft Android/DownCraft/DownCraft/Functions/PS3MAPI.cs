	...
	public class PS3MAPI
	{
		...
		public void ConnectTarget(string ip, int port = 7887)
		{
			try
			{
				...
				uint[] pid = Process.GetPidProcesses();
				this.Process.Process_Pid = pid[0];
			}
			catch (Exception ex)
			{
				Application.Current.MainPage.DisplayAlert("DownCraft", "Cannot connect to the console wrong IP ?", "OK");
			}
		}
		...
		public class PROCESS_CMD
		{
			...
			{
				public void SetMemory(uint Pid, ulong Address, byte[] Bytes)
				{
					try
					{
				...
				public string ReadString(uint Pid, ulong Address, uint Length)
				{
					PS3MAPI PS3 = new PS3MAPI();
					string stringz;
					byte[] NAME = PS3.Process.Memory.Get(Pid, Address, Length);
					stringz = Encoding.UTF8.GetString(NAME);
					while (!stringz.Contains('\0')) ;
					int length = stringz.IndexOf('\0');
					string result = stringz.Substring(0, length);
					stringz = string.Empty;
					stringz = result;
					return result;
				}

				public byte[] ReadBytes(uint Pid, ulong Address, uint Length)
				{
					PS3MAPI PS3 = new PS3MAPI();
					byte[] Bytes = PS3.Process.Memory.Get(Pid, Address, Length);
					return Bytes;
				}
			}
