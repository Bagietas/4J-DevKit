...
void DrawTextWithRectangle(const wchar_t* text, float x, float y, int* bgColor, int* textColor, bool ShadowText, bool underline)
{
	if (underline)
	{
		if (ShadowText)
		{
			...
			DrawTextWithShadow(text, x + 2, y + 2, color(textColor));
		}
		else
		{
			int RAINBOW[3] = { RainbowRED1, RainbowGREEN1, RainbowBLUE1 };
...
void DrawsnprintfText(const char* format, int* args, float x, float y, bool ShadowText, bool WithRect)
{
	if (WithRect)
	{
		if (ShadowText)
		{
			...
			DrawTextWithRectangle(woption, x, y, MC_Color::BlackTheme, MC_Color::White, true, false);
		}
		else
		{
			char option[0x100];
			...
			_sys_snprintf(option, 0x100, format, args[0], args[1], args[2], args[3], args[4]);
			StringToWideCharacter(woption, option, strlen(option));
			DrawTextWithRectangle(woption, x, y, MC_Color::BlackTheme, MC_Color::White, false, false);
		}
	}
	else
	{
		if (ShadowText)
		{
			...
			_sys_snprintf(option, 0x100, format, args[0], args[1], args[2], args[3], args[4]);
			StringToWideCharacter(woption, option, strlen(option));
			DrawTextWithShadow(woption, x, y, color(MC_Color::White));
		}
		else
		{
			char option[0x100];
